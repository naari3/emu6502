...
impl AddressingMode {
    fn fetch<T: RAMBus>(&self, cpu: &mut CPU, ram: &mut T) -> Option<u8> {
        match self {
    ...
    fn get_address<T: RAMBus>(&self, cpu: &mut CPU, ram: &mut T) -> Option<u16> {
        match self {
...
impl OpCode {
    pub fn execute<T: RAMBus>(&self, cpu: &mut CPU, ram: &mut T) {
        let ins = &self.0;
        let adr_mode = &self.1;
